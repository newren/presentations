...
\begin{frame}
  ...
  \qquad
  \begin{tabular}{lrrr}
                  & \multicolumn{1}{c}{old} & \multicolumn{1}{c}{new} \\
    few-renames:  &   18.934 s & .1892 s \\
    mega-renames: & 6424.660 s & .6573 s \\
    just-one:     &  173.093 s & .2612 s
  \end{tabular}
  }
  \only<2->{
  \qquad
  \begin{tabular}{lrrr}
                  ...
                  & \multicolumn{1}{c}{new}
                  & Speed-up \\
    few-renames:  &   18.934 s & .1892 s &  100.1 \\
    mega-renames: & 6424.660 s & .6573 s & 9774.3 \\
    just-one:     &  173.093 s & .2612 s &  662.7
  \end{tabular}
  }%
  \only<3->{
  \vspace*{\baselineskip}
...
\begin{frame}
  ...
  \begin{itemize}[<+(2)->]
    \item Just keeping both files (each of which is missing changes made
          on the other side of history)
    \item Just picking one of the two files (unknowingly discarding
    ...
    \item If users \textit{do} notice there was a rename, they
          ...
          one file to the other (unknowingly discarding the
