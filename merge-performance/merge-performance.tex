...
\begin{frame}
  ...
  \only<5->{
    ...
    {\footnotesize\texttt{%
      ...
      \quad{}...\\
      \quad{}Unmerged paths:\\
      \qquad{}	deleted by them:   buccaneer.c\\
    }}
  }
...
\begin{frame}[fragile]
  ...
  \only<2->{
    If a rename is not detected:\\[0.25em]
      ...
      \begin{tabular}{lll}
        ...
        Base:
          &
          \only<2-3|handout:0>{5eac0a57}%
          \only<4- |handout:1-2>{{\color{myred}5eac0a57}}%
          &
          00000000  \\
        Side1:
          &
          \only<2-3|handout:0>{5caff01d}%
          \only<4  |handout:1>{{\color{myred}5caff01d}}%
          \only<5- |handout:2>{{\color{mygreen}5eac0a57}}
          &
          00000000  \\
    ...
    Then:
      \begin{itemize}
        \item buccaneer.c:
              \only<2  |handout:0>{modify/delete conflict}%
              \only<3-5|handout:1>{{\color{myred}modify/delete conflict}}%
              \only<6- |handout:2>{{\color{mygreen}deleted as expected}}
        \item viking.c: totally new file
        \item
              \only<2  |handout:0>{no textual merging}%
              \only<3-6|handout:1>{\color{myred}no textual merging}%
              \only<7- |handout:2>{no textual merging \color{mygreen}needed}
      \end{itemize}
    ...
    As reported by git:\\[0.5em]
      {\footnotesize\texttt{%
        ...
        \only<3-5|handout:1>{{\color{myred}
          \quad{}Unmerged paths:\\
          \qquad{}	deleted by them:   buccaneer.c\\
        }}
        \only<6-|handout:2>{{\color{mygreen}
  ...
  \only<8|handout:2>{
    ...
    {\color{mygreen}
      ...
      unmodified by \textit{other} side of history.
    }
  }
...
\begin{frame}
  ...
  \only<3->{
    ...
    \begin{center}
    \begin{minipage}{0.5\textwidth}
    \begin{block}{\begin{center}\only<3-5|handout:0>{Improvement}\only<6>{If enough matching basenames...}\end{center}}
        \quad
        O($M*N$) $\to$
          \only<4-5|handout:0>{O((M-B)*(N-B) \only<4>{\phantom{+ B}}\only<5>{+ B})}
          \only<6>{O(minimum($M,N$))}
    \end{block}
    \end{minipage}
    \end{center}
    \vfill
